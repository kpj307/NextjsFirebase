...
export default function Home() {
  ...
  const addIItem = async(e) => {
    e.preventDefault();
    if (newItem.price !== '' && newItem.price !== ''){
      ...
      <div className="z-10 max-w-5xl w-full items-center justify-between font-mono text-sm text-white">
        ...
        <div className="bg-slate-800 p-4 rounded-lg">
          <form className="grid grid-cols-6 items-center text-black">
            ...
            <button onClick={addIItem} className="text-white bg-slate-950 hover:bg-slate-900 p-3 text-xl" type="submit">+</button>
          </form>
          <ul>
            {items.map((item, id) => (
